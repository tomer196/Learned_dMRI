...
def save_vol(reconstructions, out_dir):
    pathlib.Path(out_dir).mkdir(exist_ok=True)
    vol_data=np.zeros((144, 144, len(reconstructions), 90))
    for fname, recons in reconstructions.items():
        slice = fname[8:fname.find('.')]
        vol_data[:, :, int(slice), :] = recons.squeeze().transpose(1, 2, 0)
    name=fname[:6]
    with h5py.File(out_dir + '/' + name+'.h5', 'w') as f:
...
def reconstructe():
    ...
    for i in range(1, args.num_vol + 1):
        ...
        save_vol(reconstructions, args.out_dir)
...
def create_arg_parser():
    parser = Args()
    parser.add_argument('--test-name', type=str, default='new0.2/5/uniform_0.0001', help='name for the output dir')
